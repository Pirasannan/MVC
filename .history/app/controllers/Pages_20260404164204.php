...
class Pages extends Controller{
    ...
    private function getVideoAppointmentForRole($appointmentId, $role) {
        ...
        if ($role === 'doctor') {
            return $this->appointmentModel->getByIdForDoctor($appointmentId, $userId);
        }

        if ($role === 'patient') {
            return $this->appointmentModel->getByIdForPatient($appointmentId, $userId);
        }

        return null;
    }

    private function buildVideoCallData($appointment, $role) {
        $callId = 'appointment-' . (int)$appointment->id;
        $returnPath = $role === 'doctor' ? 'Appointments/doctor' : 'Appointments/my';
        ...
        return [
            ...
            'appointment_time' => date('M d, Y h:i A', strtotime($appointment->starts_at)),
            'appointment_type' => 'Video Consultation',
            'token_url' => URLROOT . '/Video/token/' . (int)$appointment->id,
            'return_url' => URLROOT . '/' . $returnPath,
        ];
    }

    public function doctorPrecall($appointmentId = 0) {
        if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] !== 'doctor') {
            redirect('Pages/index');
            return;
        }

        $appointment = $this->getVideoAppointmentForRole($appointmentId, 'doctor');
        if (!$appointment || strtolower($appointment->status) !== 'approved') {
            $_SESSION['flash'] = 'Only approved appointments can start a video call.';
            redirect('Appointments/doctor');
            return;
        }

        $data = $this->buildVideoCallData($appointment, 'doctor');
        
        $this->view('pages/Videoconsultation/v_doctor_precall', $data);
    }

    public function patientPrecall($appointmentId = 0) {
        if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] !== 'patient') {
            redirect('Pages/index');
            return;
        }

        $appointment = $this->getVideoAppointmentForRole($appointmentId, 'patient');
        if (!$appointment || strtolower($appointment->status) !== 'approved') {
            $_SESSION['flash'] = 'Only approved appointments can join a video call.';
            redirect('Appointments/my');
            return;
        }

        $data = $this->buildVideoCallData($appointment, 'patient');
        
        $this->view('pages/Videoconsultation/v_patient_precall', $data);
    }

    public function doctorVideoCall($appointmentId = 0) {
        if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] !== 'doctor') {
            redirect('Pages/index');
            return;
        }
        ...
        $appointment = $this->getVideoAppointmentForRole($appointmentId, 'doctor');
        if (!$appointment || strtolower($appointment->status) !== 'approved') {
            $_SESSION['flash'] = 'Appointment is not available for video calling.';
            redirect('Appointments/doctor');
            return;
        }
    ...
    public function patientVideoCall($appointmentId = 0) {
        if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] !== 'patient') {
            redirect('Pages/index');
            return;
        }
