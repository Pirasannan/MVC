...
class VideoCall extends Controller {
    ...
    public function precall($appointmentId = null) {
        ...
        // Enrich with profile images for avatar rendering on pre-call screens.
        $apt->patient_profile_image = $this->userModel->getUserProfileImage((int)$apt->patient_id) ?? '';
        $apt->doctor_profile_image = $this->userModel->getUserProfileImage((int)$apt->doctor_id) ?? '';
    ...
    public function room($appointmentId = null) {
        ...
        // Prefix with role to guarantee Stream user IDs are globally unique
        // across doctor/patient tables that may share numeric IDs.
        $streamUserId = $role . '_' . (string)$userId;
