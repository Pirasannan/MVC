...
class Appointment {
    ...
    public function getByIdForDoctor($appointmentId, $doctorId) {
        $this->db->query("SELECT a.*, p.name AS patient_name, d.name AS doctor_name
                          ...
                          LIMIT 1");
        $this->db->bind(':appointment_id', (int)$appointmentId);
        $this->db->bind(':doctor_id', (int)$doctorId);
        return $this->db->single();
    }

    public function getByIdForPatient($appointmentId, $patientId) {
        $this->db->query("SELECT a.*, p.name AS patient_name, d.name AS doctor_name
                          FROM appointments a
                          JOIN Users p ON p.id = a.patient_id
                          JOIN Users d ON d.id = a.doctor_id
