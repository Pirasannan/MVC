    ...
    try {
        ...
        participantSub = streamCall.state.remoteParticipants$.subscribe(participants => {
            ...
            if (participants.length > 0) {
                ...
                console.log('Remote participant detected:', p.name, p.sessionId);
                
                // Subscribe to the videoStream$ observable
                if (p.videoStream$) {
                    p.videoStream$.subscribe(stream => {
                        console.log('Remote video stream received:', stream ? 'active' : 'null');
                        if (stream) bindStream(remoteEl, stream);
                    });
                } else {
                    console.warn('videoStream$ not available on participant');
                }
