...
class Appointment
{
    ...
    public function getTodaysAppointmentsByPatient($patient_id)
    {
        $this->db->query("SELECT a.*, u2.name AS doctor_name
                          FROM appointments a
                          JOIN Users u2 ON u2.id = a.doctor_id
    ...
    public function getUpcomingAppointmentsByPatient($patient_id, $limit = 5)
    {
        $this->db->query("SELECT a.*, u2.name AS doctor_name
                          FROM appointments a
                          JOIN Users u2 ON u2.id = a.doctor_id
    ...
    public function getTodaysAppointmentsByDoctor($doctor_id)
    {
        $this->db->query("SELECT a.*, u1.name AS patient_name
                          FROM appointments a
                          JOIN Users u1 ON u1.id = a.patient_id
                          WHERE a.doctor_id = :did 
                          AND DATE(a.starts_at) = CURDATE()
                          ORDER BY a.starts_at ASC");
        $this->db->bind(':did', $doctor_id);
        return $this->db->resultSet();
    }

    public function getUpcomingAppointmentsByDoctor($doctor_id, $limit = 5)
    {
        $this->db->query("SELECT a.*, u1.name AS patient_name
                          FROM appointments a
                          JOIN Users u1 ON u1.id = a.patient_id
