...
$current_page = $data['current_page'] ?? 'doctorMedicalrecords';
$stats = $data['stats'] ?? [];
...
function renderRecordCard($record)
{
    ...
    <div class="record-card" id="record-card-<?= $id ?>">
        ...
        <div class="record-meta">
            <span>Patient: <?= htmlspecialchars($record->patient_name) ?></span>
            ...
            <div class="filters-section">
                <div class="filter-row">
                    <div class="search-box">
                        ...
                        <input type="text" id="searchInput" placeholder="Search by report name or patient name...">
                    </div>
                ...
                <div class="section-content" id="medical-records-list">
                    ...
                    <?php if (empty($records)): ?>
                        <div class="empty-state"
                            ...
                            <h3 style="color: #475569;">No Medical Records Found.</h3>
                            <p style="color: #64748b;">No patients have shared their medical records with you yet.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
    ...
    function filterCards() {
        ...
        document.querySelectorAll('.tab-content[style="display: block;"], .tab-content:not([style*="display: none"])').forEach(tab => {
            tab.querySelectorAll('.record-card').forEach(card => {
                ...
                const typeText = card.querySelector('.record-badge').textContent.toLowerCase();
                const patientText = card.querySelector('.record-meta span:first-child').textContent.toLowerCase();

                const matchesSearch = title.includes(term) || patientText.includes(term);
