    ...
    <?php if (!empty($data['pending_reschedules'])): ?>
      <div class="reschedule-alert">
        <div class="alert-content">
          ...
          <p>You have <?= $data['pending_reschedules'] ?> appointment<?= $data['pending_reschedules'] > 1 ? 's' : '' ?>
            with new time proposals from doctors. Please review and respond below.</p>
        </div>
      </div>
    <?php endif; ?>
                    ...
                    <td>
                      ...
                      <?php if ($rescheduleStatus === 'pending_patient' && $proposedTime): ?>
                        <div class="reschedule-proposal">
                          ...
                          <div class="proposal-actions">
                            <a href="<?= URLROOT ?>/Appointments/reschedule_accept/<?= $a->id ?>" class="btn btn-approve"
                              onclick="return confirm('Accept this new appointment time?')">Accept</a>
                            <a href="<?= URLROOT ?>/Appointments/reschedule_decline/<?= $a->id ?>" class="btn btn-reject"
                      ...
                      <?php elseif (strtolower($a->status) === 'approved'): ?>
                        <a href="<?= URLROOT ?>/VideoCall/precall/<?= $a->id ?>" class="join-consultation-btn"
                          style="display: inline-block; padding: 8px 16px; background: #4a90e2; color: white; text-decoration: none; border-radius: 4px; font-size: 14px; font-weight: 500;">
          ...
          <div class="form-grid">
            <div class="field">
              ...
              <div class="suggest-wrap">
                <input class="input" type="text" id="doctor_name" autocomplete="off" placeholder="Type a doctor name…"
                  required>
